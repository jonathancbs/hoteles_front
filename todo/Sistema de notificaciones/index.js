...
contenedorBotones.addEventListener('click', (e) => {
	...
	if (tipo === 'exito') {
		agregarToast({ tipo: 'exito', titulo: 'Congratulations !!!', descripcion: 'Gracias por la preferencia.', autoCierre: true });
	}
	if (tipo === 'error') {
		agregarToast({ tipo: 'error', titulo: 'Oh no !!!', descripcion: 'Trabajaremos para solucionar su problema.', autoCierre: true });
	}
	if (tipo === 'info') {
		agregarToast({ tipo: 'info', titulo: 'Modernize', descripcion: 'Trabajando para la modernización de nuestro Hotel !!! Gracias por la observación' });
	}
	if (tipo === 'warning') {
		agregarToast({ tipo: 'warning', titulo: 'Warning !!!', descripcion: 'Estamos mejorando para la satifacción de usted !!!' });
	}
});
